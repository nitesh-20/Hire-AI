"""HirePilot AI CLI: profile -> fetch -> prefilter -> screen -> draft -> digest -> mail.
...
def _load_profile(cfg: dict, allow_sample: bool) -> dict | None:
    ...
    if allow_sample and sample.exists():
        ...
        print("    Build the real one: python -m hirepilot profile --resume resume.pdf")
        return json.loads(sample.read_text(encoding="utf-8"))

    print(f"missing {path} — run `python -m hirepilot profile --resume <file>` first")
    return None
