...
```bash
git clone https://github.com/<owner>/HirePilot_AI.git
cd HirePilot_AI
```
...
# or without limit once configured:
...
```bash
python -m hirepilot applied "greenhouse:cloudflare:7462799"   # the id is in the digest
...
| `No module named hirepilot` | wrong folder, or venv not active | `cd` into the project root, re-activate `.venv` |
| `GEMINI_API_KEY is not set` | `.env` missing, or the key line is blank | check `.env` sits next to `config.yaml` |
| `No module named hirepilot` | wrong folder, or venv not active | `cd` into the project root, re-activate `.venv` |
| `gemini stopped early (finishReason=MAX_TOKENS)` | reasoning models spend output tokens thinking | raise the ceilings in `hirepilot/llm.py` |
